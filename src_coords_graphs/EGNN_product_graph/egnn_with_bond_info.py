...
class GCL(nn.Module):
    ...
    def edge_model(
        ...
        if edge_attr is None:
            ...
            out = torch.cat(
                [source, target, edge_attr.to(target.device)], dim=1
            )  # noqa
...
class EquivariantUpdate(nn.Module):
    ...
    def coord_model(
        ...
        input_tensor = torch.cat(
            [h[row], h[col], edge_attr.to(h.device)], dim=1
        )  # noqa
        if self.tanh:
            trans = (
                coord_diff
...
class EquivariantBlock(nn.Module):
    ...
    def forward(
        ...
        distance, coord_diff = coord2diff(x, edge_index, self.norm_constant)
        if self.sin_embedding is not None:
            distance = self.sin_embedding(distance)
        # Add Edge information here instead of the distance twice # noqa
        edge_attr = torch.cat([distance, edge_attr], dim=1)
