...
from EGNN.egnn_node_masks import EGNN 
from EGNN.utils import remove_mean, remove_mean_with_mask, assert_mean_zero_with_mask

class EGNN_dynamics_QM9(nn.Module):
    def __init__(self, in_node_nf, context_node_nf,
                 n_dims, out_node, hidden_nf=64, device='cpu',
        ...
        if mode == 'egnn_dynamics':
            self.egnn = EGNN(
                ...
                aggregation_method=aggregation_method, out_node_nf=out_node, coords_range=10)
    ...
    def _forward(self, t, xh, node_mask, edge_mask, context):
        ...
        if self.mode == 'egnn_dynamics':
            ...
            h_final, x_final = self.egnn(h.to(xh.device), x.to(xh.device), edges, node_mask=node_mask.to(xh.device), edge_mask=edge_mask.to(xh.device))
...
if __name__ == "__main__":
    ...
    # Load the dataset: 
    direc = "Dataset/data/Clean_Geometries"
    ...
    model = EGNN_dynamics_QM9(in_node_nf=in_node_nf, context_node_nf=context_nf, n_dims=n_dims, out_node=out_node, sin_embedding=True, n_layers=5, device=device)  # Something is going wrong with the embedding of H
    print("Model Setup")
    ...
    # Setup an edge mask which represents all the possible connections between the atoms: 
    ...
    for epoch in range(epochs):
        ...
        for batch, node_masks in tqdm(train_loader):
            ...
            assert_mean_zero_with_mask(batch[:, :, -3:], node_masks.unsqueeze(2).expand(batch[:, :, -3:].size()))

            true_coordinates = batch[:, :, -3:]

            true_noise = torch.randn_like(true_coordinates) * node_masks.unsqueeze(2).expand(true_coordinates[:, :, -3:].size())

            # true_noise -= noise_mean# * node_masks.unsqueeze(2).expand(true_noise[:, :, -3:].size())
            true_noise = remove_mean_with_mask(true_noise, node_masks.unsqueeze(2).expand(true_noise[:, :, -3:].size()))
            # true_noise *= node_masks.unsqueeze(2).expand(true_noise[:, :, -3:].size())
            assert_mean_zero_with_mask(true_noise[:, :, -3:], node_masks.unsqueeze(2).expand(true_noise[:, :, -3:].size()))

            noisy_coordinates = true_coordinates + true_noise
            assert_mean_zero_with_mask(noisy_coordinates[:, :, -3:], node_masks.unsqueeze(2).expand(noisy_coordinates[:, :, -3:].size()))
            ...
            h_final, x_final = model._forward(t=t, xh=batch.to(device) , node_mask=node_masks.to(device), edge_mask=edge_mask.to(device), context=None)
